...
function getActiveTasks(req, res, next) {
    db
        .any(
            "SELECT A.* FROM tasks A LEFT JOIN tasks_completed B ON A.id = B.task_id WHERE B.task_id IS NULL AND A.apt_id=${apt_id}",
            {
...
function getActiveRecurringTasks(req, res, next) {
    db
        .any(
            "SELECT A.* FROM tasks_recurring A LEFT JOIN tasks_recurring_completed B ON A.id = B.task_id WHERE B.task_id IS NULL AND A.apt_id=${apt_id}",
            {
                apt_id: req.params.apt_id
            }
        )
...
function getActiveExpensesByUser(req, res, next) {
    db
        .any(
            "SELECT A.* FROM expenses A LEFT JOIN payments_expenses B ON A.id = B.expense_id WHERE B.expense_id IS NULL AND (A.payer_id=${user_id} OR A.payee_id=${user_id})",
            {
...
function getActiveRecurringExpensesByUser(req, res, next) {
    db
        .any(
            "SELECT A.* FROM expenses_recurring A LEFT JOIN payments_recurring_expenses B ON A.id = B.expense_id WHERE B.expense_id IS NULL AND (A.payer_id=${user_id} OR A.payee_id=${user_id})",
            {
                user_id: req.params.user_id
            }
        )
...
function getActiveApartmentGoals(req, res, next) {
    db
        .any(
            "SELECT * FROM goals_apartment WHERE apt_id=${apt_id} AND (is_active=TRUE)",
            {
                apt_id: req.params.apt_id
            }
        )
